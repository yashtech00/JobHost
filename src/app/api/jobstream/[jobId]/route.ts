...
export async function GET(req: NextRequest, { params }: { params: { jobId: string } }) {  
    const { jobId } = params;  // Extracting job ID from parameters  
    ...
    try {  
      const job = await prisma.job.findUnique({  
        where: {  
          id: params?.jobId, // Find the job by ID   
        },  
      });  
  ...
  export async function PUT(
    ...
    try {
      const { userId, jobId } = params; // Correctly destructuring jobId
      const body = await req.json(); // Get request body
  
      // Ensure body only contains fields that are part of the Job model
      const { title, description, company, jobtype, location, salary } = body;
